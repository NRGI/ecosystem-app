...
use <- read.csv("https://docs.google.com/spreadsheets/d/1b4Ok08F8iBQFc8jDhsbnUSEc1kjHwwB2mFLKCj5-GbE/pub?gid=0&single=true&output=csv", stringsAsFactors = FALSE)
...
use <- use[, c("Project","Country","Organization","Data Type","Product Type","Year","Overview", "Case Study","Data.type")]
# 
...
shinyServer(function(input, output) {
  ...
  observeEvent(input$show, {
    showModal(modalDialog(
      title = "FAQs",
      HTML("Welcome to NRGI's Data Use Case Ecosystem Explorer. <br><br>
           
           This tool showcases the different uses of data in extractive governance projects from around the globe. It has been designed with an emphasis on the types of
           data a project uses and what the final product is, so that other stakeholders may find examples which they can follow and build upon. <br><br>
           
           Data use cases are projects that use extractive data to answer or pose a question. You can search the catalogue of examples by country, type of data used 
           and type of final product.<br><br>
           
           There are currently over 40 projects in the data ecosystem. NRGI will be updating the database periodically and appreciates any suggestions for additions. Please email info@resourcegovernance.org with suggestions or comments.")      ))
  })
  ...
  output$productChoices <- renderUI({
    ...
    #datas <- unique(gsub("<br>","", unlist(strsplit(paste(dat()[!(dat()[[3]]%in%c(" ","","<br>"))][[3]], collapse=''), split='<br>'))))
    
    datas <- unique(gsub("<br>","", unlist(strsplit(paste(dat()[!(dat()[["Product Type"]]%in%c(" ","","<br>"))][["Product Type"]], collapse=''), split='<br>'))))
    ...
    selectizeInput(inputId = "product.type.check",
                   label="Select one or more product types",
                   choices=datas[order(datas)],
                   multiple=TRUE,
                   selected=NULL)
  })
  ...
  productTypeList <- reactive({
    ...
    for(i in 1:length(productType())) {
      a[[i]] <- grep(productType()[i], dat()[["Product Type"]])#dat()$Product.type)
    }
    a
  })
